...
import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression  # Ajout de l'importation nécessaire
...
app.layout = html.Div([
    ...
    html.Div(className='relative h-screen', children=[
        ...
        html.Div(className='absolute top-0 right-0 bottom-0 flex items-center justify-center',
                 style={'width': '30%', 'margin-right': '2%'}, children=[
                html.Div(className='bg-white shadow-md rounded-lg p-4', style={'width': '100%'}, children=[
                    dcc.Input(id="input-prenom", type="text", placeholder="Enter a first name",
...
def update_graph(prenom_choisi, sexe_choisi, annee_choisi, departement_choisi):
    ...
    if filtered_df.empty:
        # If no data matches the filters, return empty figures
        return {}, {}
    ...
    future_df = predict_future(filtered_df)
    fig1.add_trace(go.Scatter(x=future_df['Annee'], y=future_df['Nombre'],
...
def predict_future(filtered_df):
    if filtered_df.empty:
        return pd.DataFrame(columns=['Annee', 'Nombre'])  # Return empty DataFrame if no data

    # Group by year and sum the counts
    grouped_df = filtered_df.groupby('Annee')['Nombre'].sum().reset_index()

    # Prepare data for prediction
    X = grouped_df[['Annee']].values
    y = grouped_df['Nombre'].values

    # Initialize and fit the model (you can use any model here)
    model = LinearRegression()
    model.fit(X, y)

    # Predict future years
    future_years = np.arange(filtered_df['Annee'].max() + 1, 2051).reshape(-1, 1)
    predictions = model.predict(future_years)

    # Create DataFrame for predictions
    future_df = pd.DataFrame({'Annee': future_years.flatten(), 'Nombre': predictions})
